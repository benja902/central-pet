...
import { DOM_SELECTORS, CSS_CLASSES, ANIMATION_DURATIONS, STORAGE_KEYS, WHATSAPP_CONFIG } from '../utils/constants.js';
import { 
    safeQuerySelector, 
...
class CartUI {
    ...
    cacheElements() {
        this.elements = {
            ...
            cartCustomerName: safeQuerySelector(DOM_SELECTORS.cartCustomerName),
    ...
    setupEventListeners() {
        ...
        safeAddEventListener(this.elements.cartCustomerName, 'input', (e) => {
            const customerName = e.target.value.trimStart();
            e.target.value = customerName;
            this.persistCustomerName(customerName);
            this.clearCustomerNameError();
        });
    ...
    getCustomerName() {
        if (!this.elements.cartCustomerName) {
            return '';
        }

        const customerName = this.elements.cartCustomerName.value.trim();
        this.persistCustomerName(customerName);
        return customerName;
    }

    syncCustomerNameField() {
        if (!this.elements.cartCustomerName) return;

        const savedName = localStorage.getItem(STORAGE_KEYS.customerName) || '';
        if (this.elements.cartCustomerName.value !== savedName) {
            this.elements.cartCustomerName.value = savedName;
        }
    }

    persistCustomerName(customerName) {
        try {
            if (customerName) {
                localStorage.setItem(STORAGE_KEYS.customerName, customerName);
            } else {
                localStorage.removeItem(STORAGE_KEYS.customerName);
            }
        } catch (error) {
            console.error("Error al guardar el nombre del cliente:", error);
        }
    }
    ...
    showCustomerNameError() {
        ...
        this.elements.cartCustomerName.classList.add(CSS_CLASSES.inputError);
        this.elements.cartCustomerName.focus();
        this.elements.cartCustomerName.reportValidity?.();
    }

    clearCustomerNameError() {
        if (!this.elements.cartCustomerName) return;
        this.elements.cartCustomerName.classList.remove(CSS_CLASSES.inputError);
    }
