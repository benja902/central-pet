...
class Navigation {
    ...
    /**
     * Sincroniza la categoría activa al cargar la página
     */
    syncActiveCategoryOnLoad() {
        const { hash } = window.location;

        if (hash) {
            const matchingLink = Array.from(this.elements.categoryLinks).find(
            ...
            if (matchingLink) {
                ...
                matchingLink.classList.add(CSS_CLASSES.active);
                this.scrollCategoryIntoView(matchingLink);
                return;
            }
        }
    ...
    setupScrollEffects() {
        ...
        if (this.shouldUseTransparentNavbar()) {
    ...
    /**
     * Verifica si la página debe usar navbar transparente al inicio
     */
    shouldUseTransparentNavbar() {
        if (this.isIndexPage()) return true;

        const body = document.body;
        if (!body) return false;

        return body.classList.contains('promotion-detail-page');
    }
